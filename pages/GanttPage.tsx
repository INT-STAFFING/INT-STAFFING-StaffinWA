        ...
        <div className="flex flex-col h-full">
            <div className="flex-shrink-0">
                <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                    ...
                    <div className="flex items-center space-x-2 bg-muted dark:bg-dark-muted p-1 rounded-md">
                        {(['month', 'quarter', 'year'] as ZoomLevel[]).map(level => (
                            <button key={level} onClick={() => setZoom(level)}
                                className={`px-3 py-1 text-sm font-medium rounded-md capitalize ${zoom === level ? 'bg-card dark:bg-dark-background text-primary dark:text-dark-sidebar-foreground shadow' : 'text-muted-foreground dark:text-dark-muted-foreground'}`}>
                        ...
                        {sortedAndFilteredProjects.map(project => {
                            ...
                            return (
                                <div key={project.id} className="grid grid-cols-[300px_1fr] border-b border-border dark:border-dark-border group">
                                    <div className="p-3 border-r border-border dark:border-dark-border sticky left-0 bg-card dark:bg-dark-card z-10 group-hover:bg-muted dark:group-hover:bg-dark-muted/50">
                                    ...
                                    <div className="relative">
                                        ...
                                        {project.startDate && project.endDate && (
                                            <div className="absolute h-3/5 top-1/2 -translate-y-1/2 rounded-md bg-primary hover:bg-primary group/bar" style={barStyle}>
                                            <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-dark-background text-dark-foreground text-xs rounded py-1 px-2 opacity-0 group-hover/bar:opacity-100 transition-opacity z-10 whitespace-nowrap">
