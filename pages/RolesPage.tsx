...
const RolesPage: React.FC = () => {
    ...
    const openModalForEdit = (role: Role) => { setEditingRole(role); setIsModalOpen(true); };
        ...
        <div>
            ...
            <div className="mb-6 p-4 bg-white dark:bg-gray-800 rounded-lg shadow">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                    ...
                    <select name="seniorityLevel" value={filters.seniorityLevel} onChange={handleFilterChange} className="w-full form-select"><option value="">Tutti i livelli</option>{seniorityLevels.map(s => <option key={s.id} value={s.value}>{s.value}</option>)}</select>
                        ...
                        {sortedRoles.map(role => {
                            ...
                            if (isEditing) {
                                ...
                                <tr key={role.id} className="block md:table-row p-4 md:p-0 border-b dark:border-gray-600">
                                    <td data-label="Ruolo"><input type="text" name="name" value={inlineEditingData!.name} onChange={handleInlineFormChange} className="w-full form-input p-1"/></td>
                                    <td data-label="Seniority"><select name="seniorityLevel" value={inlineEditingData!.seniorityLevel} onChange={handleInlineFormChange} className="w-full form-select p-1">{seniorityLevels.sort((a,b)=>a.value.localeCompare(b.value)).map(s=><option key={s.id} value={s.value}>{s.value}</option>)}</select></td>
                                    <td data-label="Costo"><input type="number" step="0.01" name="dailyCost" value={inlineEditingData!.dailyCost} onChange={handleInlineFormChange} className="w-full form-input p-1"/></td>
                            ...
                            return (
                            <tr key={role.id} className="block md:table-row p-4 md:p-0 border-b dark:border-gray-600">
                                <td data-label="Ruolo">{role.name}</td>
                                <td data-label="Seniority">{role.seniorityLevel}</td>
                                <td data-label="Costo">{formatCurrency(role.dailyCost)}</td>
                                <td className="text-right">
                                    <div className="flex items-center justify-end space-x-2">
                                        <button onClick={() => openModalForEdit(role)}><PencilIcon className="w-5 h-5"/></button>
                                        <button onClick={() => handleStartInlineEdit(role)}><PencilSquareIcon className="w-5 h-5"/></button>
                                        <button onClick={() => deleteRole(role.id!)}><TrashIcon className="w-5 h-5"/></button>
                                    </div>
                                </td>
                            </tr>
                        )})}
