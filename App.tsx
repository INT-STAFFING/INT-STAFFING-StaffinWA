...
const Header: React.FC<HeaderProps> = ({ onToggleSidebar }) => {
    ...
    return (
        <header className="flex-shrink-0 bg-primary-light dark:bg-primary-dark shadow-md border-b border-gray-200 dark:border-white/10">
            <div className="flex items-center justify-between p-4">
                <button onClick={onToggleSidebar} className="text-gray-500 dark:text-gray-300 focus:outline-none md:hidden">
                    <Bars3Icon className="h-6 w-6" />
                </button>
                <h1 className="text-xl font-semibold text-primary-dark dark:text-primary-light md:text-2xl">{getPageTitle(location.pathname)}</h1>
    ...
    if (loading) {
        return (
            <div className="flex items-center justify-center w-full h-full">
                <svg className="animate-spin h-10 w-10 text-accent-teal" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
