...
const ProjectsPage: React.FC = () => {
    ...
    const getStatusBadgeClass = (status: string | null): string => {
        switch (status) {
            case 'Completato': return 'bg-success/10 text-success dark:bg-success/20 dark:text-success';
            case 'In pausa': return 'bg-warning/10 text-warning dark:bg-warning/30 dark:text-warning';
            case 'In corso': return 'bg-primary/10 text-primary-darker dark:bg-primary/20 dark:text-dark-sidebar-foreground';
    ...
    const columns: ColumnDef<EnrichedProject>[] = [
        { header: 'Nome Progetto', sortKey: 'name', cell: p => (
            <div className="flex items-center">
                <span className="font-medium text-foreground dark:text-dark-foreground">{p.name}</span>
        ...
        { header: 'Risorse Assegnate', sortKey: 'assignedResources', cell: p => <span className="text-sm text-center font-semibold text-muted-foreground dark:text-dark-muted-foreground">{p.assignedResources}</span> },
     ...
     const renderRow = (project: EnrichedProject) => {
        ...
        return (
            <tr key={project.id} className="hover:bg-muted dark:hover:bg-dark-muted/50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex items-center justify-end space-x-3">
                        <button onClick={() => openModalForEdit(project)} className="text-muted-foreground hover:text-primary" title="Modifica Dettagli"><span className="text-xl">✏️</span></button>
                        <button onClick={() => handleStartInlineEdit(project)} className="text-muted-foreground hover:text-success" title="Modifica Rapida"><span className="text-xl">✏️</span></button>
        ...
        return (
            <div key={project.id} className="p-4 rounded-lg shadow-md bg-card dark:bg-dark-card">
                <div className="flex justify-between items-start">
                    <div>
                     ...
                     <div className="flex items-center space-x-1 flex-shrink-0 ml-4">
                        <button onClick={() => openModalForEdit(project)} className="p-1 text-muted-foreground hover:text-primary"><span className="text-xl">✏️</span></button>
                        <button onClick={() => handleStartInlineEdit(project)} className="p-1 text-muted-foreground hover:text-success"><span className="text-xl">✏️</span></button>
                ...
                <div className="mt-4 pt-4 border-t border-border dark:border-dark-border grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div><p className="text-muted-foreground dark:text-dark-muted-foreground">Risorse Assegnate</p><p className="font-medium text-foreground dark:text-dark-foreground">{project.assignedResources}</p></div>
    ...
    const filtersNode = (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            ...
            <div className="flex items-center space-x-4">
                <div className="flex items-center">
                    <input id="unstaffed-filter" type="checkbox" checked={showOnlyUnstaffed} onChange={(e) => setShowOnlyUnstaffed(e.target.checked)} className="h-4 w-4 rounded border-border text-primary focus:ring-primary" />
                    <label htmlFor="unstaffed-filter" className="ml-2 block text-sm text-foreground dark:text-dark-muted-foreground">Solo senza staff</label>
                </div>
            ...
            {editingProject && (
                <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={'id' in editingProject ? 'Modifica Progetto' : 'Aggiungi Progetto'}>
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            <label className="block text-sm font-medium text-foreground dark:text-dark-muted-foreground mb-1">Nome Progetto *</label>
                            <input type="text" name="name" value={editingProject.name} onChange={handleChange} required className="w-full form-input"/>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-medium text-foreground dark:text-dark-muted-foreground mb-1">Cliente</label>
                            ...
                            <div>
                                <label className="block text-sm font-medium text-foreground dark:text-dark-muted-foreground mb-1">Contratto Associato</label>
                        ...
                        <div className="grid grid-cols-2 gap-4 items-end">
                            <div>
                                <label className="block text-sm font-medium text-foreground dark:text-dark-muted-foreground mb-1">Budget (€)</label>
                                <input type="number" step="0.01" name="budget" value={editingProject.budget} onChange={handleChange} className="w-full form-input"/>
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-foreground dark:text-dark-muted-foreground mb-1">% Realizzazione</label>
                         ...
                         <div>
                            <label className="block text-sm font-medium text-foreground dark:text-dark-muted-foreground mb-1">Project Manager</label>
                        ...
                        <div className="flex justify-end space-x-3 pt-4">
                            ...
                            <button type="submit" disabled={isActionLoading('addProject') || isActionLoading(`updateProject-${'id' in editingProject ? editingProject.id : ''}`)} className="flex justify-center items-center px-4 py-2 bg-primary text-dark-foreground dark:text-dark-sidebar-foreground rounded-md hover:bg-primary-darker disabled:bg-primary/50">
