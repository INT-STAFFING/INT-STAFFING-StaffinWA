...
const ReadonlyDailyTotalCell: React.FC<{
  resource: Resource;
  date: string;
  isNonWorkingDay: boolean;
}> = React.memo(({ resource, date, isNonWorkingDay }) => {
  ...
  const total = assignments
    ...
    .reduce((sum, a) => sum + (allocations[a.id!]?.[date] || 0), 0);
...
const WorkloadHeader: React.FC<{ timeColumns: TimeColumn[] }> = ({
  timeColumns,
}) => (
...
const ResourceRow = React.memo(
  ({
    ...
    return (
      <div
        style={style}
        className="flex border-b border-gray-200 dark:border-gray-700 bg-gray-100/50 dark:bg-gray-900/50 font-bold"
      >
  ...
  const timeColumns: TimeColumn[] = useMemo(() => {
    ...
    if (viewMode === 'day') {
      return getCalendarDays(d, 21).map((day) => {
        ...
        const isNonWorkingHeader =
          day.getDay() === 0 ||
          day.getDay() === 6 ||
          !!companyCalendar.find(
  ...
  const handlePrev = useCallback(
    () =>
      setCurrentDate((prev) =>
        addDays(
          prev,
          viewMode === 'day' ? -14 : viewMode === 'week' ? -7 * 4 : -180
        )
      ),
    [viewMode]
  );
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-grow mt-4">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden h-[720px] flex flex-col">
          <WorkloadHeader timeColumns={timeColumns} />
