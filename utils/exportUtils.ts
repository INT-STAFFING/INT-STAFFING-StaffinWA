...
export const exportCoreEntities = async (data: EntitiesContextType) => {
    ...
    const projectsSheetData = projects.map(p => ({
        ...
        'Data Inizio': formatDateForExport(p.startDate),
        'Data Fine': formatDateForExport(p.endDate),
    ...
    const calendarSheetData = companyCalendar
        ...
        .map(e => ({
            'Nome Evento': e.name,
            'Data': formatDateForExport(e.date),
            'Tipo': e.type,
...
export const exportUsersPermissions = async (users: AppUser[], permissions: RolePermission[], resources: Resource[]) => {
    ...
    const permissionsData = permissions.map(p => ({
        'Ruolo': p.role,
        'Pagina': p.pagePath,
        'Accesso Consentito': p.isAllowed ? 'SI' : 'NO'
    }));
...
export const exportTemplate = async (type: ExportType | string) => {
    ...
    switch (type) {
        case 'core_entities':
            ...
            XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([["name", "email", "roleName", "function", "industry", "location", "Tutor", "Competenze", "hireDate", "workSeniority", "notes"]]), 'Risorse');
