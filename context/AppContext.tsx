...
export const AppProviders: React.FC<any> = ({ children, planningWindow, configKeys }) => {
    ...
    const markNotificationAsRead = useCallback(async (id?: string) => {
        try {
            ...
            await apiFetch(url, { method: 'PUT', body: JSON.stringify({ version: 1 }) }); // Dummy version for notifications
    ...
    const forceRecalculateAnalytics = async () => {
        ...
        try {
            const res = await apiFetch<{ data: Record<string, unknown> }>('/api/resources?entity=analytics_cache&action=recalc_all', { method: 'POST' });
            setAnalyticsCache(res.data);
            addToast('Analisi ricalcolate con successo', 'success');
        } catch (e) { addToast('Errore nel ricalcolo', 'error'); } finally { setActionLoading('recalculateAnalytics', false); }
    };
    ...
    const updateRateCard = async (rateCard: RateCard) => { setActionLoading(`updateRateCard-${rateCard.id}`, true); try { const updated = await apiFetch<RateCard>(`/api/resources?entity=rate_cards&id=${rateCard.id}`, { method: 'PUT', body: JSON.stringify(rateCard) }); setRateCards(prev => prev.map(rc => rc.id === rateCard.id ? updated : rc)); addToast('Listino aggiornato', 'success'); } catch (e: any) { addToast(e.message, 'error'); } finally { setActionLoading(`updateRateCard-${rateCard.id}`, false); } };
    ...
    const updateProjectExpense = async (expense: ProjectExpense) => { setActionLoading(`updateProjectExpense-${expense.id}`, true); try { const updated = await apiFetch<ProjectExpense>(`/api/resources?entity=project_expenses&id=${expense.id}`, { method: 'PUT', body: JSON.stringify(expense) }); setProjectExpenses(prev => prev.map(e => e.id === expense.id ? updated : e)); addToast('Spesa aggiornata', 'success'); } catch (e: any) { addToast(e.message, 'error'); } finally { setActionLoading(`updateProjectExpense-${expense.id}`, false); } };
    ...
    const updateBillingMilestone = async (milestone: BillingMilestone) => { setActionLoading(`updateBillingMilestone-${milestone.id}`, true); try { const updated = await apiFetch<BillingMilestone>(`/api/resources?entity=billing_milestones&id=${milestone.id}`, { method: 'PUT', body: JSON.stringify(milestone) }); setBillingMilestones(prev => prev.map(m => m.id === milestone.id ? updated : m)); addToast('Milestone aggiornata', 'success'); } catch (e: any) { addToast(e.message, 'error'); } finally { setActionLoading(`updateBillingMilestone-${milestone.id}`, false); } };
    
    // CORRETTO: Ora punta a /api/resources con entity=billing_milestones
    ...
    const updateCalendarEvent = async (event: CalendarEvent) => { setActionLoading(`updateCalendarEvent-${event.id}`, true); try { const updated = await apiFetch<CalendarEvent>(`/api/resources?entity=company_calendar&id=${event.id}`, { method: 'PUT', body: JSON.stringify(event) }); setCompanyCalendar(prev => prev.map(e => e.id === event.id ? updated : e)); addToast('Evento aggiornato', 'success'); } catch (e: any) { addToast(e.message, 'error'); } finally { setActionLoading(`updateCalendarEvent-${event.id}`, false); } };
    ...
    const updateInterview = async (interview: Interview) => { setActionLoading(`updateInterview-${interview.id}`, true); try { const updated = await apiFetch<Interview>(`/api/resources?entity=interviews&id=${interview.id}`, { method: 'PUT', body: JSON.stringify(interview) }); setInterviews(prev => prev.map(i => i.id === interview.id ? updated : i)); addToast('Colloquio aggiornato', 'success'); } catch (e: any) { addToast(e.message, 'error'); } finally { setActionLoading(`updateInterview-${interview.id}`, false); } };
    
    // CORRETTO: Ora punta a /api/resources con entity=interviews
            ...
            if (contract.billingType && projectIds.length > 0) {
                 await Promise.all(projectIds.map(pid => 
                     apiFetch(`/api/resources?entity=projects&id=${pid}`, {
                         method: 'PUT',
                         body: JSON.stringify({ billingType: contract.billingType, version: 1 }) // Version 1 as newly created
                     })
                 ));
            }
    ...
    const updateContract = async (contract: Contract, projectIds: string[], managerIds: string[]) => { 
        ...
        try { 
            ...
            const oldProjects = contractProjects.filter(cp => cp.contractId === contract.id).map(cp => cp.projectId);
            const oldManagers = contractManagers.filter(cm => cm.contractId === contract.id).map(cm => cm.resourceId);

            await Promise.all([
                ...oldProjects.map(pid => apiFetch(`/api/resources?entity=contract_projects&contractId=${contract.id}&projectId=${pid}`, { method: 'DELETE' })),
                ...oldManagers.map(mid => apiFetch(`/api/resources?entity=contract_managers&contractId=${contract.id}&resourceId=${mid}`, { method: 'DELETE' }))
            ]);
    ...
    const updateSkill = async (skill: Skill) => { setActionLoading(`updateSkill-${skill.id}`, true); try { const updated = await apiFetch<Skill>(`/api/resources?entity=skills&id=${skill.id}`, { method: 'PUT', body: JSON.stringify(skill) }); await fetchData(); addToast('Competenza aggiornata', 'success'); } catch (e: any) { addToast(e.message, 'error'); } finally { setActionLoading(`updateSkill-${skill.id}`, false); } };
    ...
    const addNotificationConfig = async (config: Omit<NotificationConfig, 'id'>) => {
        ...
        try {
            const newConfig = await apiFetch<NotificationConfig>('/api/resources?entity=notification_configs', { method: 'POST', body: JSON.stringify(config) });
            setNotificationConfigs(prev => [...prev, newConfig]);
            addToast('Regola notifica creata', 'success');
        } catch (e) { addToast('Errore creazione regola notifica', 'error'); } finally { setActionLoading('addNotificationConfig', false); }
    };

    const updateNotificationConfig = async (config: NotificationConfig) => {
        setActionLoading(`updateNotificationConfig-${config.id}`, true);
        try {
            const updated = await apiFetch<NotificationConfig>(`/api/resources?entity=notification_configs&id=${config.id}`, { method: 'PUT', body: JSON.stringify(config) });
            setNotificationConfigs(prev => prev.map(c => c.id === config.id ? updated : c));
            addToast('Regola notifica aggiornata', 'success');
        } catch (e: any) { addToast(e.message, 'error'); } finally { setActionLoading(`updateNotificationConfig-${config.id}`, false); }
    };
    ...
    const deleteNotificationConfig = async (id: string) => {
        ...
        } catch (e) { addToast('Errore eliminazione regola', 'error'); } finally { setActionLoading(`deleteNotificationConfig-${id}`, false); }
    };
    ...
    const updateSidebarSections = async (sections: string[]) => {
        ...
        try {
            await apiFetch('/api/resources?entity=app-config-batch', {
                method: 'POST',
                body: JSON.stringify({ updates: [{ key: resolvedConfigKeys.sidebarSectionsKey, value: JSON.stringify(sections) }] })
            });
    ...
    const updateSidebarSectionColors = async (colors: SidebarSectionColors) => {
        ...
        try {
            await apiFetch('/api/resources?entity=app-config-batch', {
                method: 'POST',
                body: JSON.stringify({ updates: [{ key: resolvedConfigKeys.sidebarSectionColorsKey, value: JSON.stringify(colors) }] })
            });
            setSidebarSectionColors(colors);
        } finally { setActionLoading('updateSidebarSectionColors', false); }
    };
    ...
    const updateDashboardLayout = async (layout: DashboardCategory[]) => {
        ...
        try {
            await apiFetch('/api/resources?entity=app-config-batch', {
                method: 'POST',
                body: JSON.stringify({ updates: [{ key: resolvedConfigKeys.dashboardLayoutKey, value: JSON.stringify(layout) }] })
            });
    ...
    const updateRoleHomePages = async (config: Record<string, string>) => {
        ...
        try {
            await apiFetch('/api/resources?entity=app-config-batch', {
                method: 'POST',
                body: JSON.stringify({ updates: [{ key: resolvedConfigKeys.roleHomePagesKey, value: JSON.stringify(config) }] })
            });
    ...
    // NEW: Skill Category/Macro Functions (Missing in original file)
    const addSkillCategory = async (cat: Omit<SkillCategory, 'id'>) => { setActionLoading('addSkillCategory', true); try { const newCat = await apiFetch<SkillCategory>('/api/resources?entity=skill_categories', { method: 'POST', body: JSON.stringify(cat) }); setSkillCategories(prev => [...prev, newCat]); } finally { setActionLoading('addSkillCategory', false); } };
    const updateSkillCategory = async (cat: SkillCategory) => { setActionLoading(`updateSkillCategory-${cat.id}`, true); try { const updated = await apiFetch<SkillCategory>(`/api/resources?entity=skill_categories&id=${cat.id}`, { method: 'PUT', body: JSON.stringify(cat) }); setSkillCategories(prev => prev.map(c => c.id === cat.id ? updated : c)); addToast('Categoria aggiornata', 'success'); } catch (e: any) { addToast(e.message, 'error'); } finally { setActionLoading(`updateSkillCategory-${cat.id}`, false); } };
    const deleteSkillCategory = async (id: string) => { setActionLoading(`deleteSkillCategory-${id}`, true); try { await apiFetch(`/api/resources?entity=skill_categories&id=${id}`, { method: 'DELETE' }); setSkillCategories(prev => prev.filter(c => c.id !== id)); } finally { setActionLoading(`deleteSkillCategory-${id}`, false); } };

    const addSkillMacro = async (macro: { name: string }) => { setActionLoading('addSkillMacro', true); try { const newMacro = await apiFetch<SkillMacroCategory>('/api/resources?entity=skill_macro_categories', { method: 'POST', body: JSON.stringify(macro) }); setSkillMacroCategories(prev => [...prev, newMacro]); } finally { setActionLoading('addSkillMacro', false); } };
    const updateSkillMacro = async (id: string, name: string) => { setActionLoading(`updateSkillMacro-${id}`, true); try { const updated = await apiFetch<SkillMacroCategory>(`/api/resources?entity=skill_macro_categories&id=${id}`, { method: 'PUT', body: JSON.stringify({ name }) }); setSkillMacroCategories(prev => prev.map(m => m.id === id ? updated : m)); addToast('Macro categoria aggiornata', 'success'); } catch (e: any) { addToast(e.message, 'error'); } finally { setActionLoading(`updateSkillMacro-${id}`, false); } };
    const deleteSkillMacro = async (id: string) => { setActionLoading(`deleteSkillMacro-${id}`, true); try { await apiFetch(`/api/resources?entity=skill_macro_categories&id=${id}`, { method: 'DELETE' }); setSkillMacroCategories(prev => prev.filter(m => m.id !== id)); } finally { setActionLoading(`deleteSkillMacro-${id}`, false); } };
...
// Sub-context hooks
export const useCatalogsContext = () => useEntitiesContext();
export const usePlanningContext = () => useEntitiesContext();
export const useConfigContext = () => useEntitiesContext();
export const useNotificationsContext = () => useEntitiesContext();
