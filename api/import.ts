...
// Helper to parse dates from Excel, handling both serial numbers and common string formats.
const parseDate = (dateValue: any): Date | null => {
    if (!dateValue) return null;

    // Case 1: It's an Excel serial number
    if (typeof dateValue === 'number') {
       ...
       // Create a new Date object from UTC components to avoid timezone shifts
       return new Date(Date.UTC(date_info.getUTCFullYear(), date_info.getUTCMonth(), date_info.getUTCDate()));
    }

    // Case 2: It's a string (e.g., "YYYY-MM-DD", "MM/DD/YYYY")
    if (typeof dateValue === 'string') {
        const date = new Date(dateValue);
        if (!isNaN(date.getTime())) {
            // Reconstruct the date using UTC components to avoid any timezone shifts
            // caused by the server's local time.
    ...
    try {
        ...
        if (newProjects && newProjects.length > 0) {
            for (const [index, p] of newProjects.entries()) {
                ...
                const startDate = p.startDate ? formatDateForDB(parseDate(p.startDate)) : null;
                const endDate = p.endDate ? formatDateForDB(parseDate(p.endDate)) : null;
