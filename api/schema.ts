...
export async function ensureDbTablesExist(db: VercelPool) {
    ...
    await db.sql`CREATE TABLE IF NOT EXISTS notification_configs ( id UUID PRIMARY KEY, event_type VARCHAR(50) NOT NULL, webhook_url TEXT NOT NULL, description TEXT, is_active BOOLEAN DEFAULT TRUE, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP );`;
    ...
    if (permsCheck.rows[0].count === '0') {
        ...
        const adminPages = [...managerPages, '/admin-settings', '/security-center', '/db-inspector', '/calendar', '/config', '/import', '/export', '/rate-cards', '/notification-settings'];
