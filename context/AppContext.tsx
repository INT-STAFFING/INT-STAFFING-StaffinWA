...
export const AppProviders: React.FC<any> = ({ children, planningWindow, configKeys }) => {
    ...
    const addBillingMilestone = async (milestone: Omit<BillingMilestone, 'id'>) => { try { const newMs = await apiFetch<BillingMilestone>('/api/resources?entity=billing_milestones', { method: 'POST', body: JSON.stringify(milestone) }); setBillingMilestones(prev => [...prev, newMs]); } catch (e) {} };
    const updateBillingMilestone = async (milestone: BillingMilestone) => { try { const updated = await apiFetch<BillingMilestone>(`/api/resources?entity=billing_milestones&id=${milestone.id}`, { method: 'PUT', body: JSON.stringify(milestone) }); setBillingMilestones(prev => prev.map(m => m.id === milestone.id ? updated : m)); } catch (e) {} };
    const deleteBillingMilestone = async (id: string) => { try { await apiFetch(`/api/resources?entity=billing_milestones&id=${id}`, { method: 'DELETE' }); setBillingMilestones(prev => prev.filter(m => m.id !== id)); } catch (e) {} };
    ...
    const deleteProjectSkill = async (projectId: string, skillId: string) => { try { await apiFetch(`/api/resources?entity=project_skills&projectId=${projectId}&skillId=${skillId}`, { method: 'DELETE' }); setProjectSkills(prev => prev.filter(ps => !(ps.projectId === projectId && ps.skillId === skillId))); } catch (e) {} };
    const updateSkillThresholds = async (thresholds: SkillThresholds) => { try { const updates = Object.entries(thresholds).map(([key, value]) => ({ key: `skill_threshold.${key}`, value: String(value) })); await apiFetch('/api/resources?entity=app-config-batch', { method: 'POST', body: JSON.stringify({ updates }) }); setSkillThresholds(thresholds); } catch (e) {} };
    const updatePlanningSettings = async (settings: { monthsBefore: number; monthsAfter: number }) => { try { const updates = [ { key: 'planning_range_months_before', value: String(settings.monthsBefore) }, { key: 'planning_range_months_after', value: String(settings.monthsAfter) } ]; await apiFetch('/api/resources?entity=app-config-batch', { method: 'POST', body: JSON.stringify({ updates }) }); setPlanningSettings(settings); fetchData(); } catch (e) {} };
    const addSkillCategory = async (cat: Omit<SkillCategory, 'id'>) => { try { const newCat = await apiFetch<SkillCategory>('/api/resources?entity=skill_categories', { method: 'POST', body: JSON.stringify(cat) }); setSkillCategories(prev => [...prev, newCat]); } catch (e) {} };
    const updateSkillCategory = async (cat: SkillCategory) => { try { const updated = await apiFetch<SkillCategory>(`/api/resources?entity=skill_categories&id=${cat.id}`, { method: 'PUT', body: JSON.stringify(cat) }); setSkillCategories(prev => prev.map(c => c.id === cat.id ? updated : c)); } catch (e) {} };
    const deleteSkillCategory = async (id: string) => { try { await apiFetch(`/api/resources?entity=skill_categories&id=${id}`, { method: 'DELETE' }); setSkillCategories(prev => prev.filter(c => c.id !== id)); } catch (e) {} };
    const addSkillMacro = async (macro: { name: string }) => { try { const newMacro = await apiFetch<SkillMacroCategory>('/api/resources?entity=skill_macro_categories', { method: 'POST', body: JSON.stringify(macro) }); setSkillMacroCategories(prev => [...prev, newMacro]); } catch (e) {} };
    const updateSkillMacro = async (id: string, name: string) => { try { const updated = await apiFetch<SkillMacroCategory>(`/api/resources?entity=skill_macro_categories&id=${id}`, { method: 'PUT', body: JSON.stringify({ name }) }); setSkillMacroCategories(prev => prev.map(m => m.id === id ? updated : m)); } catch (e) {} };
    const deleteSkillMacro = async (id: string) => { try { await apiFetch(`/api/resources?entity=skill_macro_categories&id=${id}`, { method: 'DELETE' }); setSkillMacroCategories(prev => prev.filter(m => m.id !== id)); } catch (e) {} };
    const addNotificationConfig = async (config: Omit<NotificationConfig, 'id'>) => { try { const newConfig = await apiFetch<NotificationConfig>('/api/resources?entity=notification_configs', { method: 'POST', body: JSON.stringify(config) }); setNotificationConfigs(prev => [...prev, newConfig]); } catch (e) {} };
    const updateNotificationConfig = async (config: NotificationConfig) => { try { const updated = await apiFetch<NotificationConfig>(`/api/resources?entity=notification_configs&id=${config.id}`, { method: 'PUT', body: JSON.stringify(config) }); setNotificationConfigs(prev => prev.map(c => c.id === config.id ? updated : c)); } catch (e) {} };
    const deleteNotificationConfig = async (id: string) => { try { await apiFetch(`/api/resources?entity=notification_configs&id=${id}`, { method: 'DELETE' }); setNotificationConfigs(prev => prev.filter(c => c.id !== id)); } catch (e) {} };
    const forceRecalculateAnalytics = async () => { try { const res = await apiFetch<{ data: Record<string, unknown> }>('/api/resources?entity=analytics_cache&action=recalc_all', { method: 'POST' }); setAnalyticsCache(res.data); } catch (e) {} };
    ...
    const addLeaveType = async (type: Omit<LeaveType, 'id'>) => { try { const newType = await apiFetch<LeaveType>('/api/resources?entity=leave_types', { method: 'POST', body: JSON.stringify(type) }); setLeaveTypes(prev => [...prev, newType]); } catch (e) {} };
    const updateLeaveType = async (type: LeaveType) => { try { const updated = await apiFetch<LeaveType>(`/api/resources?entity=leave_types&id=${type.id}`, { method: 'PUT', body: JSON.stringify(type) }); setLeaveTypes(prev => prev.map(t => t.id === type.id ? updated : t)); } catch (e) {} };
    const deleteLeaveType = async (id: string) => { try { await apiFetch(`/api/resources?entity=leave_types&id=${id}`, { method: 'DELETE' }); setLeaveTypes(prev => prev.filter(t => t.id !== id)); } catch (e) {} };
    const addLeaveRequest = async (req: Omit<LeaveRequest, 'id'>) => { try { const newReq = await apiFetch<LeaveRequest>('/api/resources?entity=leaves', { method: 'POST', body: JSON.stringify(req) }); setLeaveRequests(prev => [...prev, newReq]); } catch (e) {} };
    const updateLeaveRequest = async (req: LeaveRequest) => { try { const updated = await apiFetch<LeaveRequest>(`/api/resources?entity=leaves&id=${req.id}`, { method: 'PUT', body: JSON.stringify(req) }); setLeaveRequests(prev => prev.map(r => r.id === req.id ? updated : r)); } catch (e) {} };
    const deleteLeaveRequest = async (id: string) => { try { await apiFetch(`/api/resources?entity=leaves&id=${id}`, { method: 'DELETE' }); setLeaveRequests(prev => prev.filter(r => r.id !== id)); } catch (e) {} };
    const updatePageVisibility = async (visibility: PageVisibility) => { try { const updates = Object.entries(visibility).map(([path, onlyAdmin]) => ({ key: `page_vis.${path}`, value: String(onlyAdmin) })); await apiFetch('/api/resources?entity=app-config-batch', { method: 'POST', body: JSON.stringify({ updates }) }); setPageVisibility(visibility); } catch (e) {} };
    const updateSidebarConfig = async (config: SidebarItem[]) => { try { await apiFetch('/api/resources?entity=app-config-batch', { method: 'POST', body: JSON.stringify({ updates: [{ key: 'sidebar_layout_v1', value: JSON.stringify(config) }] }) }); setSidebarConfig(config); } catch (e) {} };
    const updateQuickActions = async (actions: QuickAction[]) => { try { await apiFetch('/api/resources?entity=app-config-batch', { method: 'POST', body: JSON.stringify({ updates: [{ key: 'quick_actions_v1', value: JSON.stringify(actions) }] }) }); setQuickActions(actions); } catch (e) {} };
    const updateSidebarSections = async (sections: string[]) => { try { await apiFetch('/api/resources?entity=app-config-batch', { method: 'POST', body: JSON.stringify({ updates: [{ key: 'sidebar_sections_v1', value: JSON.stringify(sections) }] }) }); setSidebarSections(sections); } catch (e) {} };
    const updateSidebarSectionColors = async (colors: SidebarSectionColors) => { try { await apiFetch('/api/resources?entity=app-config-batch', { method: 'POST', body: JSON.stringify({ updates: [{ key: 'sidebar_section_colors', value: JSON.stringify(colors) }] }) }); setSidebarSectionColors(colors); } catch (e) {} };
    const updateSidebarFooterActions = async (actions: SidebarFooterAction[]) => { try { await apiFetch('/api/resources?entity=app-config-batch', { method: 'POST', body: JSON.stringify({ updates: [{ key: 'sidebar_footer_actions_v1', value: JSON.stringify(actions) }] }) }); setSidebarFooterActions(actions); } catch (e) {} };
    const updateDashboardLayout = async (layout: DashboardCategory[]) => { try { await apiFetch('/api/resources?entity=app-config-batch', { method: 'POST', body: JSON.stringify({ updates: [{ key: 'dashboard_layout_v2', value: JSON.stringify(layout) }] }) }); setDashboardLayout(layout); } catch (e) {} };
    const updateRoleHomePages = async (config: Record<string, string>) => { try { await apiFetch('/api/resources?entity=app-config-batch', { method: 'POST', body: JSON.stringify({ updates: [{ key: 'role_home_pages_v1', value: JSON.stringify(config) }] }) }); setRoleHomePages(config); } catch (e) {} };
    const updateBottomNavPaths = async (paths: string[]) => { try { await apiFetch('/api/resources?entity=app-config-batch', { method: 'POST', body: JSON.stringify({ updates: [{ key: 'bottom_nav_paths_v1', value: JSON.stringify(paths) }] }) }); setBottomNavPaths(paths); } catch (e) {} };
    const fetchEvaluations = useCallback(async (resourceId?: string) => { try { const url = resourceId ? `/api/resources?entity=resource_evaluations&resourceId=${resourceId}` : `/api/resources?entity=resource_evaluations`; const data = await apiFetch<ResourceEvaluation[]>(url); setEvaluations(prev => data); } catch(e) {} }, []);
    const addEvaluation = async (evaluation: Omit<ResourceEvaluation, 'id'>) => { try { const saved = await apiFetch<ResourceEvaluation>('/api/resources?entity=resource_evaluations', { method: 'POST', body: JSON.stringify(evaluation) }); setEvaluations(prev => [...prev, saved]); } catch(e) {} };
    const updateEvaluation = async (evaluation: ResourceEvaluation) => { try { const saved = await apiFetch<ResourceEvaluation>(`/api/resources?entity=resource_evaluations&id=${evaluation.id}`, { method: 'PUT', body: JSON.stringify(evaluation) }); setEvaluations(prev => prev.map(e => e.id === evaluation.id ? saved : e)); } catch(e) {} };
    const deleteEvaluation = async (id: string) => { try { await apiFetch(`/api/resources?entity=resource_evaluations&id=${id}`, { method: 'DELETE' }); setEvaluations(prev => prev.filter(e => e.id !== id)); } catch(e) {} };
