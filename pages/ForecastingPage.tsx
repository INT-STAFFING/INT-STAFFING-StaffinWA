...
const ForecastingPage: React.FC = () => {
    ...
    return (
        <div>
            <h1 className="text-3xl font-bold text-foreground dark:text-dark-foreground mb-6">Forecasting & Capacity</h1>

            <div className="mb-6 p-4 bg-card dark:bg-dark-card rounded-lg shadow">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    <div>
                        <label className="block text-sm font-medium text-foreground dark:text-dark-muted-foreground">Horizontal</label>
                     ...
                     <div>
                        <label className="block text-sm font-medium text-foreground dark:text-dark-muted-foreground">Cliente</label>
                     ...
                     <div>
                        <label className="block text-sm font-medium text-foreground dark:text-dark-muted-foreground">Progetto</label>
                    ...
                    <button onClick={resetFilters} className="px-4 py-2 bg-muted text-foreground dark:bg-dark-muted dark:text-dark-foreground rounded-md hover:bg-muted/80 dark:hover:bg-dark-muted/80 w-full md:w-auto">Reset Filtri</button>
                </div>
            </div>


            {/* Grafico Utilizzo */}
            <div className="bg-card dark:bg-dark-card rounded-lg shadow p-6 mb-8">
                            ...
                            <div className="w-full flex-grow flex items-end justify-center">
                                ...
                                >
                                    <div className="absolute bottom-full mb-2 w-full opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                        <div className="bg-dark-background text-dark-foreground text-xs rounded py-1 px-2 mx-auto w-max">
                                            {data.utilization.toFixed(1)}%
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <p className="text-xs text-muted-foreground dark:text-dark-muted-foreground mt-2">
            ...
            <div className="bg-card dark:bg-dark-card rounded-lg shadow overflow-x-auto">
                <table className="min-w-full divide-y divide-border dark:divide-dark-border">
                    <thead className="bg-muted dark:bg-dark-muted">
                        <tr>
                            <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground dark:text-dark-muted-foreground uppercase tracking-wider">Mese</th>
                            <th className="px-6 py-3 text-right text-xs font-medium text-muted-foreground dark:text-dark-muted-foreground uppercase tracking-wider">G/U Disponibili</th>
                            <th className="px-6 py-3 text-right text-xs font-medium text-muted-foreground dark:text-dark-muted-foreground uppercase tracking-wider">G/U Allocati</th>
                            <th className="px-6 py-3 text-right text-xs font-medium text-muted-foreground dark:text-dark-muted-foreground uppercase tracking-wider">Utilizzo</th>
                            <th className="px-6 py-3 text-right text-xs font-medium text-muted-foreground dark:text-dark-muted-foreground uppercase tracking-wider">Surplus/Deficit (G/U)</th>
                        </tr>
                    </thead>
                    <tbody className="bg-card dark:bg-dark-card divide-y divide-border dark:divide-dark-border">
                        {forecastData.map((data, index) => (
                            <tr key={index} className="hover:bg-muted dark:hover:bg-dark-muted/50">
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-foreground dark:text-dark-foreground">{data.monthName}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-muted-foreground dark:text-dark-muted-foreground">{data.availablePersonDays.toFixed(0)}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-muted-foreground dark:text-dark-muted-foreground">{data.allocatedPersonDays.toFixed(1)}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-right font-semibold">
                                    <span className={data.utilization > 100 ? 'text-destructive dark:text-destructive' : data.utilization > 90 ? 'text-warning dark:text-warning' : 'text-success dark:text-success'}>
                                        {data.utilization.toFixed(1)}%
                                    </span>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-right font-semibold">
                                     <span className={data.surplusDeficit >= 0 ? 'text-success dark:text-success' : 'text-destructive dark:text-destructive'}>
