...
const DashboardPage: React.FC = () => {
  ...
  // 🔹 NUOVO: Capacità vs Domanda (storico e forecast, ultimi 5 + prossimi 3 mesi)
  const capacityVsDemandData = useMemo(() => {
    const results: {
      label: string;
      year: number;
      month: number;
    ...
    const startOffset = -5; // ultimi 5 mesi
    const endOffset = 3; // prossimi 3 mesi

    for (let offset = startOffset; offset <= endOffset; offset++) {
      const refDate = new Date(today.getFullYear(), today.getMonth() + offset, 1);
      const year = refDate.getFullYear();
      const monthIndex = refDate.getMonth(); // 0-based
      const firstDay = new Date(year, monthIndex, 1);
      const lastDay = new Date(year, monthIndex + 1, 0);

      // Capacità: giorni lavorativi disponibili di tutte le risorse attive
      let capacityDays = 0;
      activeResources.forEach((resource) => {
        if (!resource.hireDate) return;
        const hire = new Date(resource.hireDate);
        const effectiveStart = hire > firstDay ? hire : firstDay;
        let effectiveEnd = lastDay;
        if (resource.lastDayOfWork) {
          const last = new Date(resource.lastDayOfWork);
          if (last < effectiveEnd) effectiveEnd = last;
        }
        if (effectiveStart > effectiveEnd) return;
        capacityDays += getWorkingDaysBetween(
          effectiveStart,
          effectiveEnd,
          companyCalendar,
          resource.location,
      ...
      // Domanda: giorni allocati in quel mese
      let allocatedDays = 0;
      for (const assignmentId in allocations) {
        const assignment = assignmentById[assignmentId];
        if (!assignment) continue;
        ...
        for (const dateStr in allocMap) {
          ...
          if (day === 0 || day === 6) continue;
          if (isHoliday(allocDate, resource.location ?? null, companyCalendar)) continue;
          allocatedDays += allocMap[dateStr] / 100;
        }
      }

      const utilization = capacityDays > 0 ? (allocatedDays / capacityDays) * 100 : 0;
      const label = refDate.toLocaleString('it-IT', { month: 'short', year: '2-digit' });
    ...
    return results;
  }, [activeResources, allocations, assignmentById, resourceById, companyCalendar]);
  ...
  // 🔹 NUOVO: Utilizzo per Ruolo (Mese Corrente)
  const roleUtilizationData = useMemo(() => {
    const now = new Date();
    const firstDay = new Date(now.getFullYear(), now.getMonth(), 1);
    const lastDay = new Date(now.getFullYear(), now.getMonth() + 1, 0);

    const map: Record<string, { roleName: string; availableDays: number; allocatedDays: number }> = {};

    // Disponibilità per ruolo
    activeResources.forEach((res) => {
      if (!res.roleId) return;
      const key = String(res.roleId);
      if (!map[key]) {
        map[key] = {
          roleName: roleById[key]?.name || 'N/A',
          availableDays: 0,
          allocatedDays: 0,
        };
      }
      ...
      const effectiveEnd =
        res.lastDayOfWork && new Date(res.lastDayOfWork) < lastDay
          ? new Date(res.lastDayOfWork)
          : lastDay;
      if (!res.hireDate) return;
      const hire = new Date(res.hireDate);
      const effectiveStart = hire > firstDay ? hire : firstDay;
      if (effectiveStart > effectiveEnd) return;

      const avail = getWorkingDaysBetween(
        effectiveStart,
        effectiveEnd,
        companyCalendar,
        res.location,
      );
      map[key].availableDays += avail;
    });

    // Allocazioni per ruolo
    assignments.forEach((assignment) => {
      if (!assignment.resourceId) return;
      const res = resourceById[String(assignment.resourceId)];
      if (!res || !res.roleId) return;
      const key = String(res.roleId);
      if (!map[key]) {
        map[key] = {
          roleName: roleById[key]?.name || 'N/A',
      ...
      const lastDayTs = res.lastDayOfWork
        ? new Date(res.lastDayOfWork).getTime()
        : null;
      ...
      for (const dateStr in allocMap) {
        ...
        const day = d.getDay();
        if (day === 0 || day === 6) continue;
        if (isHoliday(d, res.location ?? null, companyCalendar)) continue;
        map[key].allocatedDays += allocMap[dateStr] / 100;
      }
    });

    return Object.values(map)
      .filter((r) => r.availableDays > 0)
      .map((r) => ({
        ...r,
        utilization: (r.allocatedDays / r.availableDays) * 100,
      }))
      .sort((a, b) => b.utilization - a.utilization);
  }, [activeResources, assignments, allocations, resourceById, roleById, companyCalendar]);

  // 🔹 NUOVO: Margine stimato per Cliente (basato su budget & costi stimati)
  const clientMarginData = useMemo(() => {
    const map: Record<
      string,
      {
        clientName: string;
    ...
    projects.forEach((project) => {
      if (!project.clientId || !project.id) return;
      const client = clientById[String(project.clientId)];
      if (!client) return;
      ...
      const realization = Number(project.realizationPercentage || 100) / 100;
      const revenue = budget * realization;

      const projectBudgetRecord = budgetAnalysisData.find((b) => b.id === project.id);
      const cost = projectBudgetRecord ? projectBudgetRecord.estimatedCost : 0;

      map[key].revenue += revenue;
      map[key].cost += cost;
      map[key].projectCount += 1;
    });
  ...
  }, [projects, clientById, budgetAnalysisData]);
  ...
  const {
    items: sortedCapacityVsDemand,
    requestSort: requestCapacitySort,
    sortConfig: capacitySortConfig,
  } = useSortableData(capacityVsDemandData, {
    key: 'year',
    direction: 'ascending',
  });

  const {
    items: sortedRoleUtilization,
    requestSort: requestRoleUtilSort,
    sortConfig: roleUtilSortConfig,
  } = useSortableData(roleUtilizationData, {
    key: 'utilization',
    direction: 'descending',
  });

  const {
    items: sortedClientMargin,
    requestSort: requestClientMarginSort,
    sortConfig: clientMarginSortConfig,
  } = useSortableData(clientMarginData, {
    key: 'margin',
    direction: 'descending',
  });
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        {/* 🔹 NUOVA Card: Capacità vs Domanda */}
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          <div className="flex justify-between items-start mb-4">
            <h2 className="text-xl font-semibold">
              Capacità vs Domanda (storico e forecast)
            </h2>
          </div>
          <div className="overflow-y-auto max-h-96">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-50 dark:bg-gray-700 sticky top-0">
                <tr>
                  ...
                  <SortableHeader
                    label="Capacità (G/U)"
                    sortKey="capacityDays"
                    sortConfig={capacitySortConfig}
                    requestSort={requestCapacitySort}
                  />
                  <SortableHeader
                    label="Domanda (G/U)"
                    sortKey="allocatedDays"
                    sortConfig={capacitySortConfig}
                    requestSort={requestCapacitySort}
                  />
                  <SortableHeader
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {sortedCapacityVsDemand.map((m) => (
                  <tr key={`${m.year}-${m.month}`}>
                    <td className="px-4 py-2 whitespace-nowrap text-sm">
                      {m.label}
                    </td>
                    <td className="px-4 py-2 whitespace-nowrap text-sm text-center">
                      {m.capacityDays.toLocaleString('it-IT', {
                        maximumFractionDigits: 0,
                      })}
                    </td>
                    <td className="px-4 py-2 whitespace-nowrap text-sm text-center">
                      {m.allocatedDays.toLocaleString('it-IT', {
                        maximumFractionDigits: 0,
                      })}
                    </td>
                    <td
                    ...
                    >
                      {m.utilization.toLocaleString('it-IT', {
                        maximumFractionDigits: 1,
                      })}
                      %
                    </td>
        ...
        {/* 🔹 NUOVA Card: Utilizzo per Ruolo */}
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          <h2 className="text-xl font-semibold mb-4">
            Utilizzo per Ruolo (Mese Corrente)
          </h2>
          <div className="overflow-y-auto max-h-96">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-50 dark:bg-gray-700 sticky top-0">
                <tr>
                  ...
                  />
                  <SortableHeader
                    label="G/U Disponibili"
                    sortKey="availableDays"
                    sortConfig={roleUtilSortConfig}
                    requestSort={requestRoleUtilSort}
                  />
                  <SortableHeader
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {sortedRoleUtilization.map((r) => (
                  <tr key={r.roleName}>
                    <td className="px-4 py-2 whitespace-nowrap text-sm font-medium">
                      {r.roleName}
                    </td>
                    <td className="px-4 py-2 whitespace-nowrap text-sm text-center">
                      {r.availableDays.toLocaleString('it-IT', {
                        maximumFractionDigits: 1,
                      })}
                    </td>
                    <td className="px-4 py-2 whitespace-nowrap text-sm text-center">
                      {r.allocatedDays.toLocaleString('it-IT', {
                        maximumFractionDigits: 1,
                      })}
                    </td>
                    <td
                    ...
                    >
                      {r.utilization.toLocaleString('it-IT', {
                        maximumFractionDigits: 1,
                      })}
                      %
                    </td>
        ...
        {/* 🔹 NUOVA Card: Margine stimato per Cliente */}
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          <h2 className="text-xl font-semibold mb-4">Margine Stimato per Cliente</h2>
          <div className="overflow-y-auto max-h-96">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-50 dark:bg-gray-700 sticky top-0">
                <tr>
