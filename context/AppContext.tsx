...
export const AppProviders: React.FC<any> = ({ children, planningWindow, configKeys }) => {
    ...
    const deleteBillingMilestone = async (id: string) => { setActionLoading(`deleteBillingMilestone-${id}`, true); try { await apiFetch(`/api/billing_milestones?id=${id}`, { method: 'DELETE' }); setBillingMilestones(prev => prev.filter(m => m.id !== id)); } finally { setActionLoading(`deleteBillingMilestone-${id}`, false); } };
    ...
    const deleteInterview = async (id: string) => { setActionLoading(`deleteInterview-${id}`, true); try { await apiFetch(`/api/interviews?id=${id}`, { method: 'DELETE' }); setInterviews(prev => prev.filter(i => i.id !== id)); } finally { setActionLoading(`deleteInterview-${id}`, false); } };
